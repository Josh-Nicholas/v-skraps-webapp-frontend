...
export class PortfolioPerformanceComponent implements OnInit {
  ...
  constructor(private messageService: MessageService, private dateService: DateService) {
    this.results['1H'][0].series = dateService.getHourDetail();
    this.results['1D'][0].series = dateService.getDayDetail();
    this.results['7D'][0].series = dateService.getWeekDetail();
    this.results['1M'][0].series = dateService.getMonthDetail();
    this.results['1Y'][0].series = dateService.getYearDetail();
  }

  ngOnInit() {
    this.messageService.getTransactions().subscribe(data => {
      this.results = data;
    })
  }
}
